...
class ProblemsetCache:
    ...
    async def get_problemset(self, contest_id):
        ...
        if not problemset:
            for i in range(10):
                await self.update_for_contest(contest_id)
                problemset = self.cache_master.conn.fetch_problemset(contest_id)
                if problemset:
                    break
            if not problemset:
    ...
    async def generate_ranklist(self, contest_id, *, fetch_changes=False, predict_changes=False):
        ...
        if fetch_changes:
            ...
            if is_rated:
                ...
                prevRatings={change.handle:change.oldRating for change in changes}
                ranklist.set_prevRatings(prevRatings)
            ...
            if cf_common.is_nonstandard_contest(contest) or has_teams:
                ...
                ranklist.set_prevRatings(current_rating)
